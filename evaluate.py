...
def ffwd_different_dimensions(in_data, out_path, checkpoint_dir,
            device_t=DEVICE, batch_size=4):

    in_path_of_shape = defaultdict(list)
    out_path_of_shape = defaultdict(list)
    shape = "%dx%dx%d" % get_img(in_data).shape
    in_path_of_shape[shape].append(in_data)
    out_path_of_shape[shape].append(out_path)
...
def process(in_path, out_path, checkpoint_dir, batch_size=2):
    #parser = build_parser()
    ...
    path = img.filename
    if height > newSize or width > newSize:
        divisor = newSize / np.maximum(height, width)
        print(str(divisor))
    ...
    ffwd_different_dimensions(in_path, out_path, checkpoint_dir, batch_size=batch_size)
